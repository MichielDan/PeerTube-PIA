...
describe('PIA tracker exposure', () => {
  it('should avoid exposing raw peer IP addresses through the built-in tracker', () => {
    ...
    const usesRawIpAssignments = /params\.httpReq\.ip|params\.ip/.test(trackerSource)
    const logsPeerIps = /logger\.(warn|debug)\('Peer %s made abnormal requests \(%d\).', ip/.test(trackerSource)

    const trackerMasksPeerIps = usesRawIpAssignments === false && logsPeerIps === false

    expect(usesRawIpAssignments || logsPeerIps, 'tracker.ts should handle peer IP data for this assertion').toBe(true)
